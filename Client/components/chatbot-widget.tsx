    ...
    <>
      ...
      {!isOpen && (
        <button
          ...
          className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-50 w-14 h-14 bg-accent text-accent-foreground rounded-full shadow-lg hover:shadow-xl transition-all hover:scale-105 flex items-center justify-center"
        >
      ...
      {isOpen && (
        <div className={cn(
          "fixed z-50 bg-card shadow-2xl flex flex-col transition-all duration-300",
          // Premium Mobile Adaptation: Occupies full screen on mobile devices, floating window layout on desktops
          isExpanded 
            ? "inset-0 md:inset-4 lg:inset-8 md:rounded-xl md:border md:border-border" 
            : "inset-0 md:inset-auto md:bottom-6 md:right-6 md:w-[380px] md:h-[600px] md:max-h-[80vh] md:rounded-xl md:border md:border-border"
        )}>
          {/* Header */}
          <div className="flex items-center justify-between p-4 border-b border-border shrink-0 bg-card">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-sm md:text-base">AR Shopping Assistant</h3>
            ...
            <div className="flex items-center gap-1">
              {/* Desktop-only Expand/Minimize functionality */}
              <Button 
                variant="ghost" 
                size="icon" 
          ...
          <div 
            ref={scrollRef}
            className="flex-1 overflow-y-auto p-4 space-y-4 scroll-smooth max-w-full"
          >
            {messages.map((message) => (
              <div key={message.id} className={cn(
                ...
                <div className={cn(
                  ...
                  <div className={cn(
                    "rounded-xl px-4 py-2 max-w-[88%] md:max-w-[85%] whitespace-pre-line shadow-sm break-words",
                  ...
                  {message.products && message.products.length > 0 && (
                    <div className="flex gap-2 overflow-x-auto pb-2 w-full no-scrollbar touch-pan-x">
                  ...
                  {message.quickReplies && message.quickReplies.length > 0 && (
                    <div className={cn("flex flex-wrap gap-2 pt-1", message.role === 'user' && "justify-end")}>
                      {message.quickReplies.map(reply => (
                        <button key={reply} onClick={() => handleSend(reply)}
                          className="text-xs px-3 py-1.5 rounded-full border border-border bg-background hover:bg-muted active:scale-95 transition-all shadow-sm">
                          {reply}
                        </button>
                      ))}
                    </div>
          ...
          {/* Input Area */}
          <div className="p-4 border-t border-border shrink-0 bg-card pb-safe-bottom">
            <form onSubmit={(e) => { e.preventDefault(); handleSend() }} className="flex gap-2">
              <Input 
                ...
                className="flex-1 text-sm md:text-base focus-visible:ring-accent"
              />
